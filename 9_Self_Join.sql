...
SELECT id, name
FROM stops AS s
INNER JOIN route AS r ON s.id = r.stop
...
SELECT r1.num, r1.company,s1.name, r4.num,r4.company FROM route AS r1
INNER JOIN route AS r2 ON r1.num = r2.num AND r1.company = r2.company
INNER JOIN stops AS s1 ON r2.stop = s1.id
INNER JOIN route AS r3 ON s1.id = r3.stop
